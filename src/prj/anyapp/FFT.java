...
public final class FFT
{
	...
	public void transform(float[] buffer, double[] im)
	{
...
//				throw new IllegalArgumentException("data array smaller than fft window size");
//			else
//				fft(buffer, im, FFT_FORWARD);
//			break;
//		case FFT_INLINE_POWER_PHASE:
//			if(im.length < windowSize)
//				throw new IllegalArgumentException("data array smaller than fft window size");
//			else
//				powerPhaseIFFT(buffer, im);
//			break;
//		case FFT_MAGNITUDE:
//			magnitudeFFT(buffer);
//			break;
...
//				throw new IllegalArgumentException("data array smaller than fft window size");
//			else
//				powerPhaseFFT(buffer, im);
//			break;
//		case FFT_REVERSE:
//			if(im.length < windowSize)
//				throw new IllegalArgumentException("data array smaller than fft window size");
//			else
//				fft(buffer, im, FFT_REVERSE);
//			break;
//		}
	}
	 ...
	 *  @param buffer        the real part of the complex input and output data
	 ...
	 *  not a power of 2
	 */
	private void fft(float[] buffer, float[] im, int direction)
	{
		int n = buffer.length;
		...
		for(int m = 1; m <= bits; m++)
		{
			...
			float Wjk_r = 1;
			float Wjk_i = 0;
			float theta = twoPI / localN;
			float Wj_r = (float) Math.cos(theta);
			float Wj_i = (float) (direction * Math.sin(theta));
			int nby2 = localN / 2;
			for (j = 0; j < nby2; j++)
			{
				...
				{
					int id = k + nby2;
					float tempr = Wjk_r * buffer[id] - Wjk_i * im[id];
					float tempi = Wjk_r * im[id] + Wjk_i * buffer[id];
					buffer[id] = buffer[k] - tempr;
					im[id] = im[k] - tempi;
					buffer[k] += tempr;
					im[k] += tempi;
				}
...
//		fft(re, im, FFT_FORWARD);
	...
	/** Computes the power spectrum of a real sequence (in place).
	 *  @param buffer the real input and output data; length must be a power of 2
	 */
	private void normalizedPowerFFT(float[] buffer)
	{
		float[] im = new float[buffer.length];
		float r, i;
		...
		{
			r = buffer[j] / windowFunctionSum * 2;
...
//		fft(pow, ph, FFT_REVERSE);
	 ...
	 *  @param buffer   the array of input data, also used for output
	 *  @param window the values of the window function to be applied to data
	 */
	private void applyWindowFunction(float[] buffer)
	{
		if(windowFunctionType != WND_NONE)
		{
			for (int i = 0; i < buffer.length; i++)
				buffer[i] *= windowFunction[i];
		}
	}
